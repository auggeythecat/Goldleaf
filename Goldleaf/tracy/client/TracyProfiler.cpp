...
#if !defined _WIN32 && !defined __linux__ && !defined __APPLE__ && !defined __SWITCH__
...
#  endif
    ...
    ptr += sprintf( ptr, "OS: Nintendo Switch\n" );
#else
...
#    define _POSIX_LOGIN_NAME_MAX 256
#  endif
...
#  endif
    ...
    if( !dataPortSearch )
    {
        ...
        if( !isListening )
        {
            fprintf( stderr, "[Tracy] Failed to listen on port %u\n", dataPort );
        }
    }
    else
    {
        for( uint32_t i=0; i<20; i++ )
        {
            if( listen.Listen( dataPort+i, 4 ) )
            {
                dataPort += i;
                isListening = true;
                fprintf( stderr, "[Tracy] Successfully listening on port %u\n", dataPort );
                break;
            }
        }
        if( !isListening )
        {
            fprintf( stderr, "[Tracy] Failed to listen on any port in range %u-%u\n", dataPort, dataPort + 19 );
        }
    }
    if( !isListening )
    {
...
#  endif
    ...
    fprintf( stderr, "[Tracy] Waiting for incoming connections on port %u...\n", dataPort );
    for(;;)
    {
        ...
        for(;;)
        {
            ...
            if( m_sock )
            {
                fprintf( stderr, "[Tracy] Accepted incoming connection\n" );
                break;
            }
...
#  endif
        ...
        // Handshake
        {
            ...
            {
                fprintf( stderr, "[Tracy] Handshake shibboleth mismatch or read error\n" );
                m_sock->~Socket();
                tracy_free( m_sock );
                m_sock = nullptr;
                continue;
            ...
            if( !res )
            {
                fprintf( stderr, "[Tracy] Failed to read protocol version\n" );
                m_sock->~Socket();
                tracy_free( m_sock );
                m_sock = nullptr;
                continue;
            ...
            {
                fprintf( stderr, "[Tracy] Protocol version mismatch: got %u, expected %u\n", protocolVersion, ProtocolVersion );
            ...
            fprintf( stderr, "[Tracy] Handshake successful\n" );
        }
...
#else
                // For non-Flax builds, just skip compression
                memset( etc1buf, 0, csz );
#endif
                tracy_free( fi->image );
